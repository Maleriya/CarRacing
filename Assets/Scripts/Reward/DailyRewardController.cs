...
public class DailyRewardController : BaseController
{
    ...
    public DailyRewardController(Transform placeForUI, PlayerProfileModel playerProfile,
    ...
    private void RefreshUi()
    {
        ...
        if (_isGetReward)
        {
            _dailyRewardView.TimerNewReward.text = "You can take your reward.";
        }
        else
        {
            if (_dailyRewardView.TimeGetReward != null)
            {
                ...
                _dailyRewardView.TimerNewReward.text = $"Time next reward: {timeGetReward}";
            }
        }
